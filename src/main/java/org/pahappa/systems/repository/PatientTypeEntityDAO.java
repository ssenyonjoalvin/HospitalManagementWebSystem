...
@ApplicationScoped
public class PatientTypeEntityDAO {
    ...
    public PatientTypeEntity getById(long id) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.get(PatientTypeEntity.class, id);
        }
    }

    public List<PatientTypeEntity> getAll() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("from PatientTypeEntity p order by p.id desc", PatientTypeEntity.class).list();
        }
    }
    ...
    public void delete(long id) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            transaction = session.beginTransaction();
            PatientTypeEntity entity = session.get(PatientTypeEntity.class, id);
